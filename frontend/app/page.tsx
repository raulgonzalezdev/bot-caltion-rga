...
// Declarar isAdmin y toggleAdmin sin invocar el hook directamente
const [isAdmin, setIsAdmin] = useState(false);
 // Define conditional styles based on isAdmin
...
const toggleAdmin = (adminKey: string): void => {
  ...
  return (
    <main
     className={isAdmin ? `min-h-screen p-5 bg-bg-verba text-text-verba ${fontClassName}` : `fixed inset-0 overflow-y-auto bg-white bg-opacity-75 ${fontClassName}`}
     data-theme={
        baseSetting
      ...
      {baseSetting ? (
        <div>
         {isAdmin && (
          ...
          {isAdmin && currentPage === "STATUS" && !production && (
            <StatusComponent
              fetchHost={fetchHost}
              settingConfig={baseSetting[settingTemplate]}
              APIHost={APIHost}
          ...
          {isAdmin && currentPage === "RAG" && !production && (
            <RAGComponent
              baseSetting={baseSetting}
              settingTemplate={settingTemplate}
              buttonTitle="Save"
